		 ...
		 <el-table
	      ...
	      <el-table-column
	   	...
	   	<div class="details" @mouseover="" v-if="flag">
	   		...
	   		<p><span>卧室数量：</span>{{details.bedroomcount}}</p>
	   		<p><span>楼层：</span>{{details.floor}}</p>
	   		<p><span>面积：</span>{{details.houseArea}}平方米</p>
	   		<p><span>朝向：</span>{{details.direction}}</p>
	   		<p><span>价格：</span>{{details.price}}元/月</p>
	   		...
	   		<p><span>配套设置：</span><span class="facility" v-for="item in details.facility">{{item}}</span></p>
...
<script>
    ...
    async created(){
    	let tal=await this.$http.get('/findLet')
    ...
    methods:{
    	...
    	formattingDate(time){
	     ...
	     async  handleDelete(index, row) {
    		let adminData=await this.$http.delete('/deleteLet?_id='+row._id)
    			let tal=await this.$http.get('/findLet')
		    	console.log(tal.data)
		    	this.tableData=tal.data
